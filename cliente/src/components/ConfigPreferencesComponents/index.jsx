  ...
  /* useEffect(()=>{
	...
	useEffect(() => {
		if (filteredHourJSON) {
			const obj = {
				totalSeats: filteredHourJSON[0].seats.length,
				reservedSeatsAmount: filteredHourJSON[0].seats.filter(
					(e) => e === true
				).length,
			};
	...
	const updatePreferencesObj = (content) => {		
		if(preferences){
			if(content.date) setPreferences({...preferences, date:`${content.date.dayNumber}/${content.date.monthNumber}`})
			if(content.hour) setPreferences({...preferences, hour:content.hour})
			if(content?.seats?.length>0) setPreferences({...preferences, seats:content.seats})
		}
	}
		...
		<section className="d-flex flex-column gap-5">
			...
			>
				...
				<button 
					...
					id="PROVISORIO" 
					onClick={()=>sendPreferencesToUpdateTableDB()}>MANDARLO!</button>
				...
				<button
					...
					onClick={() => {
						...
						if (
							...
							hourSelection &&
							dataPrices &&
							seatsCodeSelectionArray.length ===
								getSumAmountTicket() &&
